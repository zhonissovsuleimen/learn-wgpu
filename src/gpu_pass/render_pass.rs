...
impl GpuPass for RenderPass {
  fn run(
    ...
    let pipeline = self.pipeline.get_or_insert_with(|| RenderPass::init_pipeline(device, particles, window));
...
impl RenderPass {
  fn init_pipeline(device: &Device, buffer: &BufferWrapper, window: &WindowWrapper) -> RenderPipeline {
